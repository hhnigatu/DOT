...
def set_entity_breakdown( df, column, entities):
    ...
    for ind in tqdm.tqdm(df.index):
        ...
        for entity in entities:
            for i, val in enumerate(x in re.sub('[%s]' % re.escape(string.punctuation),' ', entity.lower()).split() for x in re.sub('[%s]' % re.escape(string.punctuation),' ', df['Text'][ind].lower()).split()):
                if(val):
                    entityintext.add(re.sub('[%s]' % re.escape(string.punctuation),' ', df['Text'][ind]).split()[i])
        for entity in entities:
            if all(x in list(entityintext) for x in re.sub('[%s]' % re.escape(string.punctuation),' ', entity).split()):
...
def set_entity_whole(df, column, entities):
    ...
    for ind in tqdm.tqdm(df.index):        
        ...
        for entity in entities:
            ...
            if re.search(re.sub('[%s]' % re.escape(string.punctuation),' ', entity), re.sub('[%s]' % re.escape(string.punctuation),' ', df['Text'][ind].lower())) or re.search(re.sub('[%s]' % re.escape(string.punctuation),' ', ''.join(entity.split())), re.sub('[%s]' % re.escape(string.punctuation),' ', df['Text'][ind].lower())):
                # print('found')
...
def set_entities_doc(page_df, doc_df, column):
    ...
    for ind in doc_df.index:
        entities=set()
        for entityList in list(page_df.loc[page_df['File']==doc_df['File'][ind]][column]):  
            try:
...
def set_text_doc(page_df, doc_df, column):
    for ind in doc_df.index:
        text=''
        for textVal in list(page_df.loc[page_df['File']==doc_df['File'][ind]][column]):  
            text+=textVal
...
def highlight_text(df,column, col, list_):
        ...
        for ind, sentence in zip(df.loc[df[col].notnull()].index, df.loc[df[col].notnull()][col]):
            for name in set(list_) & set(sentence):
                sentence = str(sentence).replace(name, f'<span style="color: red;">{name}</span>')
...
def uncollapse_images(collected_hash, columns= ['Path', 'File', 'Date', 'Name', 'Text', 'CaseNumber', 'Label'] ):
...
import shutil
def move_images_to_cat(path, extracted_info_df, labels=['image', 'other', 'narrative',  'handwriting', 'interview']):
    for root, dir, files in os.walk(path):
        for lable in labels:
            os.mkdir(root+'/'+lable)
        for file in files:
            try:
                target_label=extracted_info_df.loc[extracted_info_df['Path']==root+'/'+file]['label'].values[0]
...
def read_from_csv(path='doc_data.csv', labels=['Name', 'Date', 'CaseNumber', 'Label'], drop=True):

    doc_data=pd.read_csv(open(path, 'r'))
    if drop:
        doc_data=doc_data.drop(['Unnamed: 0'],1)
    for label in labels:
        for idx, val in zip(doc_data.loc[doc_data[label].notnull()].index, doc_data.loc[doc_data[label].notnull()][label]):
            entity=val.split('\', \'')
...
def get_page_num(page_df, doc_df):
    ...
    for file in files:
        ...
        # if len(pages)>0:
        try:
            first_page, last_page = pages.tolist()[0].split('/')[-1].split('-')[-1], pages.tolist()[-1].split('/')[-1].split('-')[-1]
            first_page, last_page = re.sub('.jpg', '', first_page).lstrip('0'), re.sub('.jpg', '', last_page).lstrip('0')
...
def split_large_files(doc_data, extracted_info_df, minNum=4):
    ...
    for indx in doc_data.loc[doc_data['CaseNumber'].notnull()].index:
        ...
        if len(doc_data['CaseNumber'][indx])>minNum:
            large_files.append(extracted_info_df.loc[extracted_info_df['File']==doc_data['File'][indx]])
            doc_data = doc_data.drop(indx)
    large_files_df=pd.concat(large_files)
    large_files_df['File']=large_files_df['File']+ large_files_df['DocClass'].astype(str)
...
def name_and_date_match(doc_data, caseList, ID_column='CaseNumber'):
    ...
    for ind in name_list:
        ...
        for casefiles in caseList:
            ...
            match[casefiles]=find_matches(nocase['Name'][ind], casefiles.names)/len(nocase['Name'][ind])
            if not nocase['Date'].isnull()[ind]:
                # print(nocase['Date'][ind])
                match[casefiles]+=find_matches(nocase['Date'][ind], casefiles.dates)/len(nocase['Date'][ind])
...
def split_by_label(extracted_info_df, label='form'):
    ...
    for file in files:
        i=0
        same_doc=extracted_info_df.sort_values(by='Path').loc[extracted_info_df['File']==file]
        for idx, page in zip(same_doc.index, same_doc['Path']):
            if extracted_info_df['Label'][idx]==label:
                i+=1
            extracted_info_df.at[idx, 'FileSplit']=extracted_info_df['File'][idx]+str(i)+'.0'
