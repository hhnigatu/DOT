  ...
  if div_ratio!=0:
    ...
    for i in range(div_ratio):
      if i!=div_ratio-1:
        ...
        data['representative_page']=collected_hash[i*div : (i+1)*div]['representative_page'].tolist()
        dataset= Dataset.from_pandas(data)
        
      else:
        data=pd.DataFrame(columns=['representative_page'])
        data['representative_page']=collected_hash[i*div :]['representative_page'].tolist()
        dataset= Dataset.from_pandas(data)
      datas.append(data)
...
def set_features( features = Features({
...
def encode_dataset(dataset, batched=True, batch_size=1):
  encoded_dataset = dataset.map(preprocess_data, remove_columns=dataset.column_names, features=set_features(), 
...
def preprocess_data(batch):
  ...
  """
  try:
    images = [Image.open(path).convert("RGB") for path in batch['representative_page']]
...
def get_vector_rep(data, model, labels=['image', 'other', 'narrative', 'form', 'handwriting', 'interview']):
...
def update_dataframe(collected_hash, lable_outputs):
  ...
  collected_hash['doc']=collected_hash['page_path'].apply(lambda x: x.split('/')[-2])
...
def highlight_label_pages(label, collected_hash):
  collected_hash= collected_hash.sort_values(by='page_path').drop(['representative_page', 'images'], axis=1).style.apply(lambda x: ['background: red' if x.Label==label else '' for i in x], axis=1)
  return collected_hash
...
def plot_page_type_data(collected_hash, labels):
  num=[]
  for label in labels:
    num.append(sum(collected_hash.loc[collected_hash['Label']==label]['number_of_duplicates']))
  plt.bar(labels, num)
  plt.savefig('page_type_stat.png')
  plt.show()

def create_cat_folders(path, labels=['image', 'other', 'narrative',  'handwriting', 'interview', 'form']):
