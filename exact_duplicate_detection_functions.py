    ...
    for root, dirs, files in os.walk(dataset_path):
        for file in files:
            if file.endswith('.pdf'):
                try:
                    info['numOfPages'], info['numFilesEncrpted'] = info['numOfPages'] + getNumPages(os.path.join(root, file))[0], info['numFilesEncrpted'] + getNumPages(os.path.join(root, file))[1]
                except:
...
def PdfToImage(dataset_path, outputPath="images"):
    ...
    for root, dirs, files in os.walk(dataset_path):
        for tag, file in enumerate(files):
            if file.endswith('.pdf'):
                outfile=os.path.join(dataset_path, outputPath, '__'.join(root.split('/')[len(dataset_path.split('/')):] )+ '__'+ file)
                try:
                    if not os.path.exists(outfile):
                        os.mkdir(outfile)
                        convert_from_path(os.path.join(root, file), fmt='jpeg',  output_folder=os.path.join(dataset_path, outputPath, '__'.join(root.split('/')[len(dataset_path.split('/')):] )+ '__'+ file), output_file=file.rsplit( ".", 1 )[ 0 ]  + str(tag))
                    else:
                ...
                #     #     tagged.append((file.rsplit( ".", 1 )[ 0 ]  + str(tag), file))
                    ...
                    if err.errno==36:
                        ...
                        print('File name too long. Changed from:        ', os.path.join(dataset_path, outputPath, '__'.join(root.split('/')[len(dataset_path.split('/')):] )+ '__'+ file))
                        print('To:                                      ', outfile)
...
def hash_pages(dataset_path):
    ...
    for label_folder, _, file_names in os.walk(dataset_path):
        ...
        if label_folder != dataset_path and label_folder.split('/')[-1].split('.')[-1]=='pdf':
            ...
            for _, _, image_names in os.walk(label_folder):
                ...
                for image_file in image_names:
                    ...
                    data= {'page_path': label_folder+ "/" + image_file, 'hashstring': str(hashlib.md5(image).hexdigest()) }
...
def read_from_collected_hash(path_to_csv , columns=['number_of_duplicates']):
    ...
    df['pages'] = df['pages'].apply(lambda x: x[2:-2])

    df.index = df['hashstring']
...
def HashPages(dataset_path, num_rep=2, doc_index=-2):
    ...
    try: 
       ...
       # file_replica.to_csv(open('./file_replica.csv', 'w'))
        ...
        print("There is a file path mistake. Please make sure you've entered a valid file path.")
...
def find_pages_with_duplicates(hash_df, num_rep=2):
    ...
    for idx, hash in zip(hash_df.index, hash_df['hashstring']):
        ...
        if len(rep)>=num_rep: 
            ...
            hash_df.at[idx, 'duplicate_pages']=hash_df['page_path'].iloc[rep_index].tolist()
    return hash_df
...
def create_replica_df(hash_df, doc_index=-2):
    ...
    """
    replica_df=pd.DataFrame(columns=['page', 'file_name', 'duppage', 'files_with_duplicate_page', 'dupdocPages'])
    replica_df['page']=hash_df.loc[hash_df['duplicate_pages'].notnull()]['page_path']
    replica_df['duppage']=hash_df.loc[hash_df['duplicate_pages'].notnull()]['duplicate_pages']

    replica_df['file_name']=replica_df['page'].apply(lambda x: x.split('/')[doc_index])
    replica_df['files_with_duplicate_page']=replica_df['duppage'].apply(lambda x: [y.split('/')[doc_index] for y in x])
...
def collect_file_level_duplicate_info(hash_df, dataset_path, doc_index=-2):
    ...
    page_replica=create_replica_df(hash_df, doc_index)
    file_replica=pd.DataFrame(columns=['file_name', 'files_with_duplicate_page', 'percentage_match', 'matched_page_num'])
    ...
    for ind in file_replica.index:
        file_replica['files_with_duplicate_page'][ind]=Counter([doc for docs in page_replica.loc[page_replica['file_name']==file_replica['file_name'][ind]]['files_with_duplicate_page'].tolist() for doc in docs])
        file_replica['matched_page_num'][ind]=page_range[file_replica['file_name'][ind]]
    page_num=[]
    for doc in list(file_replica['file_name']):
    # print(dataset_path+'/' +doc)
        page_num.append(len([name for name in os.listdir(os.path.join(dataset_path, doc)) if os.path.isfile(os.path.join(dataset_path,doc, name))]))
...
def ranges(list_page_nums):
    for a, b in itertools.groupby(enumerate(list_page_nums), lambda pair: pair[1] - pair[0]):
        b = list(b)
        yield b[0][1], b[-1][1]
...
class document_replica_info:
    ...
    def create_replica(self, replica_path, replica_name, match_ratio, matched_pages_range, matched_page_num):
        class replica:
            def __init__(self):
                self.replica_path="/home/hellina/tutorial dataset/images/"+replica_path
...
def threshold_by_percent(file_df, min_percentage_value=0, max_percentage_value=100, min_page_in_file=1, max_page_in_file=21):
    try:
        assert(min_page_in_file>0), "File should have atleast 1 page. Please set min_page_in_file to a value greater than 0."
        assert(max_page_in_file<=21), "The largest number of pages in your dataset is 21. Please set value of max_page_in_file to less than or equal to 21."
        assert(min_percentage_value>=0 and max_percentage_value>=0 ), "Percentage value cannot be negative."
        assert(min_percentage_value<=100 and max_percentage_value<=100 ), "Percentage value cannot be exceed 100%."
        percentage_cutoff={}
        t=min_percentage_value/100
        max_percentage_value=max_percentage_value/100
        min_per=min_percentage_value/100
        percentage_cutoff[('Percentage Match', min_per*100, max_percentage_value*100, min_page_in_file, max_page_in_file)]=[]
        try:
            rep_df=file_df.loc[file_df['numberOfPagesInFile']>=min_page_in_file].loc[file_df['numberOfPagesInFile']<=max_page_in_file]
        except NameError:
            print('There is something wrong with your variable name. Please check if it has been defined and the spelling is correct.')  
        for doc, rep, page_num, page_range in zip(rep_df['file_name'], rep_df['percentage_match'], rep_df['numberOfPagesInFile'], rep_df['matched_page_num']):
            document= document_replica_info(re.sub('__','/',doc), doc.split('__')[-1], str(page_num))
            # print(document.name)
            for replicas, perc in zip(list(rep.keys()), list(rep.values())):
                    replicas=replicas.strip()
                    if (min_per<=round(perc['ratio'],2) and round(perc['ratio'],2)<=max_percentage_value) :
                            ...
                            replica=document.create_replica(re.sub('__','/',replicas),replicas.split('__')[-1], round(perc['ratio']*100,2), str(list(sorted(ranges(list(set([int(x) for x in page_range[replicas]])))))), file_df.loc[file_df['file_name']==replicas]['numberOfPagesInFile'].values[0])
                            document.add_replica(replica)
                            # print(replica.replica_name)
            percentage_cutoff[('Percentage Match', min_per*100, max_percentage_value*100, min_page_in_file, max_page_in_file)].append(document)

        # print(percentage_cutoff[0].duplicates[0].match_ratio)
        return percentage_cutoff
    except AttributeError:
        print("Make sure that variables are not written with quotes around them.")
         

def threshold_by_number_of_matched_pages(file_df, min_page_in_file_match=1, max_page_in_file_match=21, min_page_in_file=1, max_page_in_file=21):
    try:
        assert(min_page_in_file>0), "File should have atleast 1 page. Please set min_page_in_file to a value greater than 0."
        assert(max_page_in_file<=21), "The largest number of pages in your dataset is 21. Please set value of max_page_in_file to less than or equal to 21."
        assert(min_page_in_file_match>0 and max_page_in_file_match>0 ), "Number of pages cannot be negative or zero."
        assert(min_page_in_file_match<=21 and max_page_in_file_match<=21 ), "The largest number of pages in your dataset is 21. Please set value of max_page_in_file to less than or equal to 21."
    
        page_num_cutoff={}
        t=min_page_in_file_match
        page_num_cutoff[('Number of Matched Pages', min_page_in_file_match, max_page_in_file_match,min_page_in_file, max_page_in_file)]=[]
        rep_df=file_df.loc[file_df['numberOfPagesInFile']>min_page_in_file].loc[file_df['numberOfPagesInFile']<=max_page_in_file]
        ...
        for doc, rep, page_num, page_range in zip(rep_df['file_name'], rep_df['percentage_match'], rep_df['numberOfPagesInFile'], rep_df['matched_page_num']):
            ...
            for replicas, perc in zip(list(rep.keys()), list(rep.values())):
                    replicas=replicas.strip()
                    if (int(page_num*perc['ratio'])<=max_page_in_file_match and int(page_num*perc['ratio'])>=min_page_in_file_match):
                            replica=document.create_replica(re.sub('__','/',replicas),replicas.split('__')[-1], round(perc['ratio']*100,2), str(list(sorted(ranges(list(set([int(x) for x in page_range[replicas]])))))), file_df.loc[file_df['file_name']==replicas]['numberOfPagesInFile'].values[0])
                            document.add_replica(replica)
                            # print(replica.replica_name)

            page_num_cutoff[('Number of Matched Pages', min_page_in_file_match, max_page_in_file_match,min_page_in_file, max_page_in_file)].append(document)
    ...
    try:
        ...
        with open(os.path.join(outputPath,"Duplicate_Records.txt"), 'w') as f:
            ...
            # =items.items()
                ...
                # print(cond)
                # print(docs)
                f.write("*********************************************Bin " + str(bin_id) + "********************************************\n")
                print("*********************************************Bin " + str(bin_id) + "********************************************\n")
                bin_id+=1
                # if cond[1]==0.0:
                #     f.write("\t\t\t\t\t\t\t\t\t\tFiles with "+str(cond[0])+" Match " +cond[1] + "  " + str(cond[3]) + " and " + str(cond[4])+'\n')
                # else:
                f.write("\t\t\t\t\t\tFiles with "+str(cond[0])+" between  " + str(cond[1])+ "-" +str(cond[2])+ '\n')
                f.write("\t\t\t\t\t\tNumber of Pages in files between "+str(cond[3])+"-"+ str(cond[4]) +'\n')
                print("\t\t\t\t\t\tFiles with "+str(cond[0])+" between  " + str(cond[1])+ "-" +str(cond[2])+ '\n')
                print("\t\t\t\t\t\tNumber of Pages in files between "+str(cond[3])+"-"+ str(cond[4]) +'\n')
                ...
                # for doc, rep, page_num, page_range in zip(rep_df['file_name'], rep_df['percentage_match'], rep_df['numberOfPagesInFile'], rep_df['matched_page_num']):
                    ...
                    for replica in doc.duplicates:#zip(list(rep.keys()), list(rep.values())):
                            # print(replica.replica_name)
                        ...
                        # if (min_value<perc['ratio']>=t) :
                            ...
                            print("\tPercentage of pages in Doc A found in Doc B: "+ str(float(replica.match_ratio))+ "%\n")
                            print("\tNumber of pages in Doc A found in Doc B: "+ str(int(float(replica.match_ratio)/100 * float(doc.pagenum)))+ "\n")
                            ...
                            print("\tPercentage of pages in Doc B found in Doc A: "+ str(round(((float(replica.match_ratio)/100)*float(doc.pagenum))/replica.matched_page_num,2)*100)+ "%\n")
                            print("\tNumber of pages in Doc B found in Doc A: "+ str(int(float(replica.match_ratio)/100 * float(doc.pagenum)))+ "\n")

                            print("\tPath: " + replica.replica_path+ "\n")

                            f.write("____________________________________________________________________________________________________________________\n")
                            f.write("Doc A\n")
                            
                            f.write("\tName: " + doc.name +"\n")
                            f.write("\tNumber of Pages: "+ str(doc.pagenum)+ "\n")
                            f.write("\tPercentage of pages in Doc A found in Doc B: "+ str(float(replica.match_ratio))+ "%\n")
                            f.write("\tNumber of pages in Doc A found in Doc B: "+ str(int(float(replica.match_ratio)/100 * float(doc.pagenum)))+ "\n")
                            ...
                            f.write("\tNumber of Pages: "+ str(replica.matched_page_num)+ "\n")
                            f.write("\tPercentage of pages in Doc B found in Doc A: "+ str(round(((float(replica.match_ratio)/100)*float(doc.pagenum))/replica.matched_page_num,2)*100)+ "%\n")
                            f.write("\tNumber of pages in Doc B found in Doc A: "+ str(int(float(replica.match_ratio)/100 * float(doc.pagenum)))+ "\n")
...
def select_representative_pages(hash_df):
    ...
    """
    collected_hash= pd.DataFrame(hash_df.groupby('hashstring')['page_path'].apply(lambda x: '+'.join(x)))
    collected_hash['pages']=collected_hash['page_path'].apply(lambda x: x.split('+'))
    collected_hash['number_of_duplicates']=collected_hash['pages'].apply(lambda x: len(x))
    collected_hash['representative_page']=collected_hash['pages'].apply(lambda x: x[0])
...
def visualize_file_pairs(path_1, path_2, outputPath='./outputs/'):
    ...
    # hash_df=find_pages_with_duplicates(hash_df)
        ...
        # print()
        commonpages_1={}
        all_pages_1=[Image.fromarray(color_image(x, False)).resize((200,300),Image.ANTIALIAS) for x in sorted(path_1pages)]
        for pages, dups in zip(hash_df.loc[hash_df['file_name']==path_1]['page_path'], hash_df.loc[hash_df['file_name']==path_1]['duplicate_pages']):
            try:
                for dup in dups:
                    if dup.split('/')[-2]==path_2:
                        change=sorted(path_1pages).index(pages)
                        all_pages_1[change]=Image.fromarray(color_image(pages, True)).resize((200,300),Image.ANTIALIAS)
        ...
        commonpages_2={}
        all_pages_2=[Image.fromarray(color_image(x, False)).resize((200,300),Image.ANTIALIAS) for x in sorted(path_2pages)]
        for pages, dups in zip(hash_df.loc[hash_df['file_name']==path_2]['page_path'], hash_df.loc[hash_df['file_name']==path_2]['duplicate_pages']):
            try:
                for dup in dups:
                    if dup.split('/')[-2]==path_1:
                        ...
                        # print(change)
                        all_pages_2[change]=Image.fromarray(color_image(pages, True)).resize((200,300),Image.ANTIALIAS)
                        # commonpages_2[pages]=color_image(pages, True)
            except:
                pass
        # print(all_pages)
...
def show_matching_pages(path_1, path_2, duplicate_df):
    ...
    for pages, dups in zip(duplicate_df.loc[duplicate_df['file_name']==path_1]['page'], duplicate_df.loc[duplicate_df['file_name']==path_1]['duppage']):
        for dup in dups:
            # print(dup)
            if dup.split('/')[-2]==path_2:
...
def show_unique_pages_in_file(path_1, path_2):
    ...
    print("There are "+ str(len(unique_pages)) +" unique pages in " + path_1 +" that are not found in " +path_2)
...
def download_pdf(dataset_path, outputPath = 'pdf'):
    '''
    Takes all pdfs in the dataset_path and saves copies in a pdf folder
    Parameters:
    dataset_path: path to dataset
    outputPath: dataset_path/outputPath is a folder that contains all the pdfs
    '''
    if not os.path.exists(dataset_path+ '/' + outputPath):
        os.mkdir(dataset_path + '/'  + outputPath)
    outfile = os.path.join(dataset_path, outputPath)
    file_path = []
    file_name = []
    real_file_name = []
    ...
    for root, dirs, files in os.walk(dataset_path):
        for tag, file in enumerate(files):
            if file.endswith('.pdf'):
                if root.split('/')[-1] == dataset_path.split('/')[len(dataset_path.split('/'))-1]:
                    placement = file
                else:
                    placement = '__'+ file
                real_file_name.append(file)
                file_name.append(dataset_path.split('/')[len(dataset_path.split('/'))-1] + '__' + '__'.join(root.split('/')[len(dataset_path.split('/')):] ) + placement)
                file_path.append(os.path.join(root, file))

    for i in np.arange(len(file_path)):
        shutil.copy2(file_path[i], outfile)
    for i in np.arange(len(file_name)):
        os.rename(outfile + '/' + real_file_name[i], outfile + '/' + file_name[i]) 

def create_csv(dataset_path, csvPath = 'csv'):
    '''
    Creates a csv file named the "(dataset folder)__hash_df.csv" and moves it into a csv folder in the dataset_path
    Parameters:
    dataset_path: path to dataset
    csvPath: dataset_path/csvPath is a folder that contains all the csvs
    '''
    #hash_df.csv = most recent csv of the most recent datapath called using HashPages()
    #dataset_path/csvPath = csv folder for csvs specfic to the datapath

    if not os.path.exists(dataset_path+ '/' + csvPath):
        os.mkdir(dataset_path + '/'  + csvPath)
    outfile = os.path.join(dataset_path, csvPath)

    csv_files = []
    for root, dirs, files in os.walk('/Users/heatherwei/Desktop/DOT-master'):
        for tag, file in enumerate(files):
