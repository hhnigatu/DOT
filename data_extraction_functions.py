...
def extract_with_bbox(bbox, encoded_dataset, processor, lable_df):
    ...
    for idx in lable_df.index:
       possible_entities.append(get_text_from_bbox(bbox, encoded_dataset[idx]['bbox'].squeeze().tolist(), processor.tokenizer.decode(encoded_dataset[idx]['input_ids'].squeeze().tolist()).split())) 
    possible_entities= list(set([num for page in possible_entities if page is not None for num in page ]))
...
def clean_entity_list(clean_list, entityList, minLen=3, maxLen=20, maxFreq=10):
    ...
    clean_entitiy=list([" ".join(re.sub('[%s]' % re.escape(string.punctuation), '',re.sub(r'\d', '', str(token).strip())) for token in cleaner(entity.lower().strip()) if not token.is_stop) for entity in entityList])
    
    # for each word in each entity, only retain words that are more than a single letter/number
    clean_entitiy=list([" ".join(entity for entity in cl_entity.split() if len(entity)>1) for cl_entity in clean_entitiy])

    
    # for each entity only retain ones that are with in the boundry (minLen, maxLen)
    clean_entitiy=[entity for entity in clean_entitiy if len(entity.strip())>minLen and len(entity.strip())<maxLen]

    clean_entitiy=[entity[0] for entity in Counter(clean_entitiy).items() if entity[1]>maxFreq]
    print("Removed " +str(len(entityList)-len(clean_entitiy))+ " entites. There are now " + str(len(clean_entitiy)) + " entites." )
...
#     for ind, date_list in zip(doc_data.index, doc_data.loc[doc_data['Dates'].notnull()]['Dates']):
    date_set=set()
    try:
        for date in [re.sub('\.', '/', re.sub('-', '/', re.sub('\'', '', re.sub('\]','', re.sub('\[','', x))))) for x in date_list]:
#             if len(date)==8:
